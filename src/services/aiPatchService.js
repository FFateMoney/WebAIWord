...
function pickTargetId(opItem) {
  // Backward/compat aliases for LLM outputs
  const id = opItem?.target_id ?? opItem?.targetId ?? opItem?.id ?? opItem?.target
  return typeof id === 'string' && id.trim() ? id.trim() : null
}

function requirePath(opItem, op) {
  if (typeof opItem?.path !== 'string' || !opItem.path) {
    throw new Error(`${op} 操作缺少 path 字段`)
  }
  return opItem.path
}
  ...
  applyPatch(baseAiView, patchEnvelope) {
    ...
    for (const opItem of patchEnvelope.operations) {
      ...
      if (op === 'add' || op === 'replace') {
        setByPointer(next, requirePath(opItem, op), opItem.value, op)
      } else if (op === 'remove') {
        removeByPointer(next, requirePath(opItem, op))
      } else if (op === 'insert_after_id' || op === 'insert_before_id') {
        const targetId = pickTargetId(opItem)
        if (!targetId) throw new Error(`${op} 操作缺少 target_id（兼容 targetId / id）`)
        const idx = findBlockIndexById(next, targetId)
        if (idx < 0) throw new Error(`未找到目标段落 id: ${targetId}`)
        const insertIdx = op === 'insert_after_id' ? idx + 1 : idx
        body.splice(insertIdx, 0, opItem.value)
      } else if (op === 'replace_by_id') {
        const targetId = pickTargetId(opItem)
        if (!targetId) throw new Error('replace_by_id 操作缺少 target_id（兼容 targetId / id）')
        const idx = findBlockIndexById(next, targetId)
        if (idx < 0) throw new Error(`未找到目标段落 id: ${targetId}`)
        body[idx] = opItem.value
      } else if (op === 'update_by_id') {
        const targetId = pickTargetId(opItem)
        if (!targetId) throw new Error('update_by_id 操作缺少 target_id（兼容 targetId / id）')
        const idx = findBlockIndexById(next, targetId)
        if (idx < 0) throw new Error(`未找到目标段落 id: ${targetId}`)
